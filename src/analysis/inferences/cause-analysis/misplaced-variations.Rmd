...
# Inferential Analysis: Causes of Misplaced Variations

This notebook contains the inferential analysis about the causes of use case description quality.
In particular, we investigate which factors influence the number of misplaced variations in a use case.
...
```{r figdir}
figdir <- 'figures/inferential/causes/misvariations/'
```
...
```{r dag}
dag <- dagify(
        misplaced ~ cplx + author + owner,

        exposure = "cplx",
        outcome = "misplaced",

        labels = c(owner = "requirements owner", author = "requirements author", cplx = "complexity", 
                    misplaced = "misplaced variations"),
        coords = list(x = c(cplx = 0, owner = 0, author = 0, misplaced = 1),
                        y = c(cplx = 1, owner = 2, author = 0, misplaced = 1))
)
...
```{r identification}
ggdag_adjustment_set(dag, exposure = "cplx", outcome = "misplaced") +
...
```{r formula}
f <- bf(Misplaced.Variations | trials(interactions) ~ cplx + Location + (1|owner) + (1|author))
```

Since the number of misplaced variations is bounded by the number of interactions in a use case, we can model the regression with a binomial distribution.
...
d.req$cplx <- factor(d.req$cplx, levels=c("Low Complexity", "Medium Complexity", "High Complexity"), ordered=TRUE)

d.req.rel <- d.req %>% 
    select(req, cplx, owner, author)
...
m.prior <-
  brm(data = d, family = binomial, f, prior = priors,
    ...
    file = "./src/analysis/fits/cause.misplacedvariations.prior"
  )
```
...
m <-
  brm(data = d, family = binomial, f, prior = priors,
    iter = 4000, warmup = 1000, chains = 4, cores = 4,
    seed = 4, 
    file = "./src/analysis/fits/cause.misplacedvariations"
  )
```
...
```{r marginal-location}
conditional_effects(m, effects="Location")
```
