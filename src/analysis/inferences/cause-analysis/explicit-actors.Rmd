...
# Inferential Analysis: Causes of Number of Explicit Actors

This notebook contains the inferential analysis about the causes of use case description quality.
In particular, we investigate which factors influence the number of explicit actors in a use case.
...
```{r figdir}
figdir <- 'figures/inferential/causes/explicitactors/'
```
...
```{r dag}
dag <- dagify(
        actors ~ cplx + author + owner + location,
        ...
        labels = c(owner = "requirements owner", author = "requirements author", cplx = "complexity", 
                    actors = "explicit actors", location = "location"),
        coords = list(x = c(cplx = 0, owner = 0, author = 0, actors = 1, location = 1),
                        y = c(cplx = 1, owner = 2, author = 0, actors = 1, location = 0))
)
...
```{r identification}
ggdag_adjustment_set(dag, exposure = "cplx", outcome = "actors") +
...
```{r formula}
f <- bf(actors.explicit | trials(entities) ~ cplx + Location + (1|owner) + (1|author))
```

Since the number of explicit actors is bounded by the number of entities in a use case, we can model the regression with a binomial distribution.
...
Still, we add the requirements-level attributes of interest (`cplx` and `owner`) to each use case.
...
m.prior <-
  brm(data = d, family = binomial, f, prior = priors,
    ...
    file = "./src/analysis/fits/cause.explicitactors.prior"
  )
```
...
m <-
  brm(data = d, family = binomial, f, prior = priors,
    iter = 4000, warmup = 1000, chains = 4, cores = 4,
    seed = 4, 
    file = "./src/analysis/fits/cause.explicitactors"
  )
```
